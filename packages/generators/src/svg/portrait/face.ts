...
function eye(b: SvgBuilder, id: PortraitIdentity, c: FaceColors, cx: number, dir: -1 | 1, s: FaceSettings): string {
  ...
  const upper = d(
    ...
    'C',
    x(0.12),
    top + (s.outerDy > 0 ? s.outerDy : 0),
    x(0.72),
    top + (s.innerDy > 0 ? s.innerDy * 0.8 : 0),
    inner[0],
    inner[1],
  );
...
function mouth(expr: Expression, c: FaceColors): string {
  ...
  switch (expr) {
    case 'happy':
      return (
        el('path', {
          d: 'M280 399Q300 404 320 399Q316 425 300 427Q284 425 280 399Z',
          fill: dark,
          stroke: lip,
          'stroke-width': 2.5,
        }) +
    ...
    case 'angry':
      return (
        el('path', {
          d: 'M282 414Q300 398 318 414Q300 409 282 414Z',
          fill: dark,
          stroke: lip,
          'stroke-width': 2.5,
          'stroke-linejoin': 'round',
  ...
  switch (expr) {
    case 'sad': {
      const tear = (x: number, dir: number) =>
        ...
        el('path', {
          d: d('M', x + dir * 2, EYE_Y + 76, 'c', 6, 8, 6, 14, 0, 16, 'c', -6, -2, -6, -8, 0, -16, 'Z'),
          fill: '#c8ecff',
          stroke: '#6ab0e0',
          'stroke-width': 1.5,
        });
      const shine = (cx: number) =>
        el('path', {
          d: d('M', cx - 20, EYE_Y + 22, 'Q', cx, EYE_Y + 30, cx + 20, EYE_Y + 22),
          fill: 'none',
          stroke: '#bfe8ff',
          'stroke-width': 4,
          opacity: 0.9,
    ...
    case 'embarrassed': {
      ...
      return (
        ...
        hatch(362) +
        el('path', {
          d: 'M428 222C444 248 446 264 428 272C410 264 412 248 428 222Z',
          fill: '#c8ecff',
          stroke: '#5aa0d0',
          'stroke-width': 2.5,
        }) +
    ...
    case 'surprised':
      return [
        ...
        .map(([x, y, dx, dy]) =>
          el('path', {
            d: d('M', x as number, y as number, 'l', dx as number, dy as number),
            stroke: '#3a2a3a',
            'stroke-width': 5,
            'stroke-linecap': 'round',
          }),
        ...
        .map(([x, y, r]) =>
          el('path', {
            d: d(
              'M',
              x as number,
              (y as number) - (r as number),
              'Q',
              x as number,
              y as number,
              (x as number) + (r as number),
              y as number,
              'Q',
              x as number,
              y as number,
