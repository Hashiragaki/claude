...
export const anim2dParamsSchema = z.object({
  ...
  animations: z
    .array(
      z
        .string()
        .regex(/^[a-z][a-z0-9_]*$/, "Nom d'animation : minuscules, chiffres et « _ » " + '(ex. « idle », « walk »).'),
    )
    .min(1)
    .max(8)
    .default(['idle'])
...
export const keySchema = z.object({
  ...
  frame: z
    ...
    .int()
    .min(0)
    .describe('Frame index (0 = first frame; may equal `frames` ' + 'to close a loop)'),
...
export const partSchema = z.object({
  id: z
    ...
    .regex(
      ...
      'Id de pièce : lettres, chiffres, « _ » ou « - », ' + 'commençant par une lettre.',
    ),
  svg: z
    .string()
    .min(1)
    .max(20_000)
    .describe('SVG fragment (shapes only, no <svg> root) drawn in ' + 'frame coordinates, rest pose'),
  ...
  .superRefine((spec, ctx) => {
    ...
    spec.parts.forEach((p, i) => {
      if (ids.has(p.id)) {
        ctx.addIssue({
          code: 'custom',
          path: ['parts', i, 'id'],
          message: `Id de pièce en double : « ${p.id} ».`,
        });
      }
      ids.add(p.id);
      if (/<svg[\s>]/i.test(p.svg)) {
        ctx.addIssue({
          code: 'custom',
          path: ['parts', i, 'svg'],
          message: 'Un fragment de pièce ne doit pas ' + 'contenir de racine <svg>.',
        });
      }
    });
    ...
    spec.animations.forEach((a, i) => {
      if (names.has(a.name)) {
        ctx.addIssue({
          code: 'custom',
          path: ['animations', i, 'name'],
          message: `Animation en double : « ${a.name} ».`,
        });
      }
      names.add(a.name);
      a.keys.forEach((k, j) => {
        if (!ids.has(k.part)) {
          ctx.addIssue({
            code: 'custom',
            path: ['animations', i, 'keys', j, 'part'],
            message: `Pièce inconnue « ${k.part} » ` + `(pièces : ${[...ids].join(', ')}).`,
          });
        }
        if (k.frame > a.frames) {
          ctx.addIssue({
            code: 'custom',
            path: ['animations', i, 'keys', j, 'frame'],
            message: `Frame ${k.frame} hors de l'animation ` + `« ${a.name} » (0 à ${a.frames}).`,
          });
        }
      });
    });
    ...
    if (cols * spec.width > 4096 || spec.animations.length * spec.height > 4096) {
      ctx.addIssue({
        ...
        path: ['width'],
        message:
          'Planche trop grande (4096 px maximum ' + 'par côté) : réduisez la taille ou le nombre ' + 'de frames.',
      });
    }
  });
