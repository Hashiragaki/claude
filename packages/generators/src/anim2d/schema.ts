...
const vec2 = z
  .array(z.number())
  .length(2, 'Attendu : un tableau de 2 nombres [x, y].');
...
export const keySchema = z.object({
  ...
  frame: z
    ...
    .min(0)
    .describe(
      'Frame index (0 = first frame; may equal `frames` ' +
        'to close a loop)',
    ),
  translate: vec2
    .optional()
    .describe('Offset [dx, dy] in pixels from the rest pose'),
  rotate: z
    .number()
    .optional()
    .describe('Rotation in degrees around the pivot (clockwise)'),
...
export const anim2dSpecSchema = z
  ...
  .superRefine((spec, ctx) => {
    ...
    spec.parts.forEach((p, i) => {
      ...
      if (/<svg[\s>]/i.test(p.svg)) {
        ctx.addIssue({
          code: 'custom',
          path: ['parts', i, 'svg'],
          message: 'Un fragment de pièce ne doit pas ' +
            'contenir de racine <svg>.',
        });
      }
    });
      ...
      a.keys.forEach((k, j) => {
        ...
        if (k.frame > a.frames) {
          ctx.addIssue({
            ...
            message: `Frame ${k.frame} hors de l'animation ` +
              `« ${a.name} » (0 à ${a.frames}).`,
          });
        }
      });
    ...
    const cols = Math.max(
      ...spec.animations.map((a) => a.frames),
    );
    if (cols * spec.width > 4096 ||
        spec.animations.length * spec.height > 4096) {
      ctx.addIssue({
        code: 'custom',
        path: ['width'],
        message: 'Planche trop grande (4096 px maximum ' +
          'par côté) : réduisez la taille ou le nombre ' +
          'de frames.',
      });
    }
  });
