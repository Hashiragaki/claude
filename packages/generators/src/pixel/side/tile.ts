...
// Copie volontairement dupliquée de `pixel/tileset/tile.ts` : on ne dépend pas de ce dossier pour
// rester strictement dans `pixel/side/*` (voir plan de reprise), avec la palette du plateformer.
...
export class Tile extends PixelCanvas {
  ...
  constructor(
    readonly pal: SideTilesetPalette,
    readonly theme: SideTilesetTheme,
    role: string,
  ) {
    super(T, T);
...
/** Applique une opacité uniforme à toute la tuile (pour l'eau semi-transparente). */
export function withOpacity(t: Tile, alpha: number): void {
  const a = Math.round(alpha * 255);
  for (let y = 0; y < T; y++) {
    for (let x = 0; x < T; x++) {
      const c = t.get(x, y);
      if (c[3] > 0) t.set(x, y, [c[0], c[1], c[2], a]);
    }
  }
}
