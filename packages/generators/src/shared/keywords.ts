...
function wordPattern(word: string): RegExp {
  ...
  return new RegExp(`(^| )${w}(s|x|es)?( |$)`);
}

const patternCache = new Map<string, RegExp>();

/**
 * Renvoie la clé dont un mot-clé apparaît le plus tôt dans le texte (mot entier, pluriel en
 * « s »/« x » accepté), ou `undefined`. La comparaison ignore accents et casse : `épées` trouve `epee`.
 */
