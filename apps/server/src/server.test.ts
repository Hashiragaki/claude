...
describe('serveur Forge (hors-ligne)', () => {
  ...
  it('expose l\'état, les modes et les générateurs', async () => {
    ...
    const generators = (await server.app.inject('/api/generators')).json() as { id: string; params: { type: string } }[];
  ...
  it('crée un projet de démonstration avec ses assets et son planning', async () => {
    const res = await server.app.inject({ method: 'POST', url: '/api/projects', payload: { name: 'Mon VN', mode: 'vn', template: 'vn-demo' } });
    ...
    const diagnostics = (await server.app.inject(`/api/projects/${project.id}/validate`)).json() as { severity: string }[];
  ...
  it('génère un asset procédural via la file de jobs et gère les versions', async () => {
    ...
    const variant = await server.generation.generate(project.id, { generator: 'sfx', parentId: asset.id, mode: 'procedural' });
    ...
    const edit = server.generation.generate(project.id, { generator: 'sfx', parentId: asset.id, instruction: 'plus aigu' });
    await expect(edit).rejects.toThrow(/IA/);
  }, 60000);

  it('gère le planning par l\'API', async () => {
    const base = `/api/projects/${project.id}/planner`;
    const task = (await server.app.inject({ method: 'POST', url: `${base}/tasks`, payload: { title: 'Tester le prologue', estimateHours: 2 } })).json();
    expect(task.title).toBe('Tester le prologue');
    const moved = (await server.app.inject({ method: 'POST', url: `${base}/tasks/${task.id}/move`, payload: { status: 'in_progress', index: 0 } })).json();
    ...
    const bad = await server.app.inject({ method: 'PATCH', url: `${base}/tasks/${task.id}`, payload: { dependsOn: [task.id] } });
  ...
  it('crée des projets RPG et 3D valides', async () => {
    for (const [mode, template] of [
      ...
      const created = (await server.app.inject({ method: 'POST', url: '/api/projects', payload: { name: `Test ${mode}`, mode, template } })).json() as ProjectManifest;
      expect(created.assets.length).toBeGreaterThan(0);
      const diagnostics = (await server.app.inject(`/api/projects/${created.id}/validate`)).json() as { severity: string; message: string }[];
      expect(diagnostics.filter((d) => d.severity === 'error')).toEqual([]);
    }
  }, 180000);
});
...
describe('serveur Forge (assistant IA simulé)', () => {
  ...
  it('exécute les outils de l\'assistant et persiste la conversation', async () => {
    ...
    expect(request.tools?.map((t) => (t as { name: string }).name)).toEqual(expect.arrayContaining(['create_task', 'generate_asset', 'write_file']));
  ...
  it('écrit un script via l\'outil write_file et renvoie les diagnostics', async () => {
    ...
    llm.enqueue({
      content: [toolUseBlock('write_file', { path: 'scripts/script.vn', content: 'label start:\n    jump nulle_part\n' })],
    });
